...
class RiemannTensor:
    ...
    def calculate(self, config='llll', simplify=True, show=True):
        ...
        if self.config == 'ulll':
            ...
            for a  in range(self.PyCSCObj.num_coordinates):
                for b in range(self.PyCSCObj.num_coordinates):
                    for c in range(self.PyCSCObj.num_coordinates):
                        for d in range(c+1):
                            key = str(a) + str(b) + str(c) + str(d)
                            symm_key = str(a) + str(b) + str(d) + str(c)
                            ans = 0
                            for i in range(self.PyCSCObj.num_coordinates):
                                ans += contra_metric[i,a]*self.riemann_fk[str(i) + str(b) + str(c) + str(d)]
            ...
            # Using the definition takes twice as long as changing the config so we are going to calculate 'llll' config evertime 'ulll' is requested and then just change the config.
            ...
            #                 if ans != 0:
            #                     R = sym.symbols(f'R^{key[0]}_{key[1:]}')
            #                     display(Math(sym.latex(R) + ' = ' + sym.latex(self.riemann_sk[key])))